...
# User -> Travel Agent -> Flight Agent (if user asks about flights) or Hotel Agent (if user asks about hotels) . it is called handoff. The travel agent will decide when to handoff to the specialist agents based on the user's query. The specialist agents will then use their tools to provide specific recommendations for flights or hotels, and then hand back the response to the travel agent to provide a final response to the user. This allows for a more modular and specialized approach to handling different aspects of travel planning.
